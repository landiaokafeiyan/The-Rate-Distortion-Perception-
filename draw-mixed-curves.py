...
def plotC3C1():
    ...
    # bbox_to_anchor = (0., 1.02, 1., .102),
    # plt.savefig(directory  + 'paper R_D.pdf')
    plt.savefig('Corollary3Case1DR.pdf')
    plt.show()
...
def plotT4DstarR_diff():
    ...
    for r in np.linspace(start, stop, num):
        ...
        P_thresholds.append(sigmaX**2 * bb)
    # label0 = r'$\underline{D}^{\prime}$'
    label0 = r'$D_diff$'
    label1 = r'$\overline{D}(R,R_c,P|W^2_2)$'#
    ...
    # print(f'P_index={P_index},RRR={R_list[P_index[-1]]}')
...
def plotT4DstarP_diff():
    ...
    plt.savefig('theroem6_fullsize_DP_R0.1_P0.1.pdf')
    plt.show()
